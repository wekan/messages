...
class LdapResult extends LdapMessage {
   ...
   * @param {number} [options.status=0] An LDAP status code.
   * @param {string} [options.matchedDN=''] The DN that matched the request.
   * @param {string[]} [options.referrals=[]] A set of servers to query for
   * references.
   * @param {string} [options.diagnosticMessage] A message indicating why a
   * request failed.
   */
  constructor (options = {}) {
    super(options)
